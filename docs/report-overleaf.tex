...
\begin{titlepage}
  ...
  \centering
  \vspace*{1.0cm}
  \includegraphics[width=0.42\textwidth]{docs/pics/dtu.png}\par
  \vspace{1.35cm}
  {\Large\textcolor{ink}{Full stack udvikling, drift og distribuerede systemer (62595)}\par}
  \vspace{1.0cm}
  ...
  \end{minipage}
  \vfill
  {\large\underline{\href{https://github.com/kasparasdownas/Full-stack_projekt}{\textbf{Github link}}}\par}
\end{titlepage}
...
Det vigtigste tekniske problem er, at to brugere ikke må kunne booke det samme sæde samtidigt. Det løses i databasen og booking-servicen gennem transaktioner og en unik databasebegrænsning på \code{(event\_id, seat\_id)}.
...
\textbf{Kort status.} Projektet dækker de vigtigste kursusemner: serviceopdelt backend, REST/JSON, sikkerhed, Docker Compose, PostgreSQL, test, CI, deployment-runbook, CSR/SSR-valg og beskyttelse mod dobbeltbooking.
...
Gruppen startede med tre personer, men endte med to. Derfor blev scope justeret, så vi prioriterede kursuskravene og beskyttelsen mod dobbeltbooking frem for ekstra features, som ikke var nødvendige for projektet.
...
Uge 10--12 & Sikkerhed, CI, smoke-test, samtidighedstest, deployment-runbook og rapport. \\
\bottomrule
\end{tabularx}
\end{table}
...
Lektion 09--10 & Frontend- og driftsemner er brugt i routing, komponentopdeling, Docker Compose og deployment-runbook. \\
Lektion 13 & Projektet har backend-tests, frontend-tests, smoke-test, samtidighedstest, baseline-script og GitHub Actions. \\
\bottomrule
\end{tabularx}
\end{table}
...
\draw[rel, dashed] (bookings) -- node[left]{kan oprette} (email);
\end{tikzpicture}
\caption{Forenklet datamodel}
\end{figure}

\subsection{Strategi mod dobbeltbooking}
...
\subsubsection{Booking og beskyttelse mod dobbeltbooking}
...
E-mailbeskeder gemmes i \code{email\_outbox} og systemet sender derfor ikke rigtige e-mails, men gemmer de beskeder, der ville være sendt ved booking, annullering og ventelistenotifikation. Det er nok til demoen og kræver ikke en ekstern mailtjeneste.

\subsubsection{Event lifecycle}

Admin kan oprette, redigere, publicere, afpublicere, annullere og slette events, hvor events har statusværdierne \code{UNPUBLISHED}, \code{PUBLISHED} og \code{CANCELED}. Public event-listen viser kun publicerede og ikke-slettede events. Admin-dashboardet kan se alle ikke-slettede events.
...
Det normale brugerflow starter på forsiden, hvor brugeren enten kan gennemse events eller logge ind. Forsiden forklarer kort, at systemet handler om eventbooking, sædevalg og beskyttelse mod dobbeltbooking.
...
\begin{itemize}
  ...
  \item databaseintegritet og fejlscenarier med dobbeltbooking
\end{itemize}
...
Vi har udviklet et distribueret event-bookingsystem med React CSR-frontend, tre Spring Boot-services, Nginx gateway, PostgreSQL, Flyway-migrationer, Docker Compose og CI. Systemet er ikke kun en prototype med statiske data, da det har rigtige brugerflows, databasepersistens, login, rollebaseret adgang, adminfunktioner, venteliste og bookinglogik.

Den vigtigste del af projektet er beskyttelsen mod dobbeltbooking. Ved at kombinere transaktionelle skrivninger med en unik databasebegrænsning kan systemet afvise en ekstra booking, selv når to brugere forsøger at booke samme sæde samtidigt. Samtidighedstesten viser derfor noget vigtigt: løsningen virker ikke kun i brugergrænsefladen, men også når to forespørgsler rammer backend næsten samtidigt.
